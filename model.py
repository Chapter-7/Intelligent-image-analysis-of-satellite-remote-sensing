...
from tensorflow.keras.layers import (
    ...
    Conv2D, MaxPooling2D, UpSampling2D, DepthwiseConv2D,
...
class BilinearUpsampling(Layer):
    ...
    def call(self, inputs):
        if self.upsampling:
            return tf.image.resize(inputs, (inputs.shape[1] * self.upsampling[0],inputs.shape[2] * self.upsampling[1]),)
        else:
            return tf.image.resize(inputs, (self.output_size[0],self.output_size[1]))
...
def Deeplabv3(weights='pascal_voc', input_tensor=None, input_shape=(160, 160, 8), classes=6, backbone='mobilenetv2', OS=8, alpha=1.):
    ...
    model = Model(inputs, x, name='deeplabv3_') #outputs=x
    # load weights
    return model
